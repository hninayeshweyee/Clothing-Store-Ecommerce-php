...
";
    ...
    if ($purchaseQty < 1) {
        ...
        echo "<script>window.location='Clothing_Details.php';</script>";
        exit();
    }
...
function CalculateTotalAmount()
{
    ...
    // Check if 'PurchaseFunctions' is set and is an array
    if (isset($_SESSION['PurchaseFunctions']) && is_array($_SESSION['PurchaseFunctions'])) {
        $size = count($_SESSION['PurchaseFunctions']);
        ...
        for ($i = 0; $i < $size; $i++) { 
            ...
            $TotalAmount += ($PurchasePrice * $PurchaseQty);  // Add to total
        }

        return $TotalAmount;  // Return the total amount
    } else {
        // Return 0 if 'PurchaseFunctions' is not set or is empty
        return 0;
    }
}
