...
if (isset($_POST['btnUpdate'])) {
    $txtvariationID = $_POST['variationID'];
	...
	$UpdateProduct="update product_variations set productID = '$txtproductID', productPrice = '$txtproductPrice',productQuantity = '$txtproductQuantity', sizeID = '$txtsizeID', colorID = '$txtcolorID', image_1='$fileName1',image_2='$fileName2'  where variationID = '$txtvariationID'";
...
$query="select * from product_variations where variationID = $variationID";
...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3   bg-gradient-dark" id="sidenav-main">
  ...
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
    ...
    <div class="container-fluid py-4">
        <div class="col-md-5">
            <form action="Product_Detail_Update.php" method="POST" enctype="multipart/form-data">
        ...
        <label>Choose Product Name :</label>
                <div class="input-group input-group-outline mb-3">
                    ...
                    $show = "select * from product where productID = $productID";
                    $bData = mysqli_query($connect, $show);
                    $bresult = mysqli_fetch_array($bData);
                    $bName = $bresult['productName'];
                    ?>
                    <select name="txtproductID" class="form-control">
                        <option value="<?php echo $arr['productID']?>"><?php echo $bName; ?></option>
                        <?php
                ...
                <div class="input-group input-group-outline mb-3">
                    ...
                    $show = "select * from size where sizeID = $sizeID";
                    $bData = mysqli_query($connect, $show);
                    $bresult = mysqli_fetch_array($bData);
                    $bName = $bresult['sizeName'];
                    ?>
                    <select name="txtsizeID" class="form-control">
                        <option value="<?php echo $arr['sizeID']?>"><?php echo $bName; ?></option>
                        <?php

                        $brandquery = "select * from size";
                        $result = mysqli_query($connect, $brandquery);
                        $count = mysqli_num_rows($result);
                ...
                <div class="input-group input-group-outline mb-3">
                    ...
                    $show = "select * from color where colorID = $colorID";
                    $cData = mysqli_query($connect, $show);
                    $cresult = mysqli_fetch_array($cData);
                    $cName = $cresult['colorName'];
                    ?>
                    
                    <select name="txtcolorID" class="form-control">
                        <option value="<?php echo $arr['colorID']?>"><?php echo $cName; ?></option>
                        <?php

                        $brandquery = "select * from color";
                        $result = mysqli_query($connect, $brandquery);
                        $count = mysqli_num_rows($result);

                        for ($i = 0; $i < $count; $i++) {
                        $arr1 = mysqli_fetch_array($result);
                        $cName = $arr1['colorName'];
                        $cId = $arr1['colorID'];
                ...
                <input type="hidden" name="txtvariationID" class="form-control" value="<?php echo $arr['productQuantity']?>" style="background-color:white;"> 
                ...
                <input type="hidden" name="variationID" value="<?php echo $arr['variationID']?>">
                  </div>
